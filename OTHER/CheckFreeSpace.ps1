...
.DESCRIPTION
    ...
    Если Вы не хотите, чтобы при каждом выполнении скрипта на миг появлялось окошко терминала,
    
    следует конвертировать данный скрипт в .exe файл
    
    Скрипт должен выполняться в фоновом режиме, поэтому его следует добавить в планировщик задач.
    .LINK
